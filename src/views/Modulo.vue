<template>
  <div class="pb-8 pl-16">
    <div class="flex justify-between items-center pb-2">
      ...
      <h4
        class="text-center text-3xl mb-6 uppercase font-bold text-turquesa-700 cursor-pointer"
        @click="item.name = ''"
      >
        {{ datosCategoria.category.name }}
      </h4>
      <div></div>
    </div>
    <div class="grid grid-cols-2 grid-flow-col gap-4">
      ...
      <div class="col-span-1">
        ...
        <template v-else>
          <div class="flex">
            <template
              v-for="(imagen, imgIndex) in datosCategoria.category.icon"
              :key="imgIndex"
            >
              <div
                class="cardImage rounded-lg shadow-lg bg-gray-200 pb-1 p-2 relative mx-auto"
              >
                <div
                  v-if="datosCategoria.category.icon > 1"
                  class="absolute top-4 right-4 bg-turquesa-500 h-10 w-10 flex items-center justify-center rounded-full text-lg text-gray-50 font-semibold"
                >
